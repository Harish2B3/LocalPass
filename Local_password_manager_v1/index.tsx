...
const Dashboard = () => {
  ...
  const [otpData, setOtpData] = useState({ otp: null, expiry: null });
  ...
  useEffect(() => {
    const fetchOtp = async () => {
      ...
      try {
        console.log(`[Dashboard] Fetching OTP for user ${loggedInUser.id}...`);
        const response = await fetch(`${API_BASE_URL}/users/extension/otp`, {
          headers: { "X-User-ID": loggedInUser.id },
        });

        if (!response.ok) {
          const errData = await response.json();
          console.error(`[Dashboard] OTP fetch failed: ${errData.error || response.statusText}`);
          return;
        }

        const data = await response.json();
        console.log(`[Dashboard] OTP received: ${data.otp ? 'YES' : 'NONE'}`);
        setOtpData(data);
      } catch (e) {
        console.error("[Dashboard] Connection error fetching OTP:", e);
      }
    };
  ...
  return (
    <div className="page-content">
      <div className="dashboard-grid">
        ...
        {/* Browser Extension OTP */}
        <div className="card highlight-card">
          <div className="card-header">
            <svg
              className="icon"
              style={{ color: "var(--accent-color-purple, #a855f7)" }}
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
            ...
            <span>Extension OTP</span>
          </div>
          <p className="card-subtitle">Browser Extension Access Code</p>
          <div
            className="otp-code-display"
            onClick={() => {
              if (otpData.otp) {
                navigator.clipboard.writeText(otpData.otp);
                toast.addToast("OTP copied to clipboard!", "success");
              }
            }}
            style={{
              fontSize: otpData.otp ? "2.5rem" : "1.25rem",
              fontWeight: "bold",
              letterSpacing: otpData.otp ? "8px" : "normal",
              textAlign: "center",
              margin: "1rem 0",
              color: otpData.otp ? "var(--accent-color-blue)" : "var(--accent-color-red)",
              cursor: otpData.otp ? "pointer" : "default",
              padding: "1.5rem 1rem",
          ...
          >
            {otpData.otp || (otpData.message || "Access Disabled")}
          </div>
          <div className="otp-footer" style={{ fontSize: "0.8rem", color: "var(--text-secondary-color)", textAlign: "center" }}>
            {otpData.otp ? (
              <>Expires in ~{Math.ceil((new Date(otpData.expiry).getTime() - Date.now()) / 60000)} mins</>
            ) : (
              <span onClick={() => { }} style={{ cursor: 'pointer', textDecoration: 'underline' }}>
                Enable in Settings &gt; Network
              </span>
            )}
          </div>
        </div>
...
const VaultCarousel = () => {
  ...
  const fetchPassword = async (id) => {
    try {
      const response = await fetch(`${API_BASE_URL}/vault/reveal-password`, {
        method: "POST",
        headers: {
      ...
      toast.addToast("Failed to retrieve password.", "error");
    }
    return null;
  };
  ...
  const handleCopy = async (text: string, type: "Username" | "Password", id?: number) => {
    let textToCopy = text;
    if (type === "Password" && text === "******" && id !== undefined) {
      const realPassword = await fetchPassword(id);
      if (realPassword) {
        textToCopy = realPassword;
        // Update local state so we don't fetch again
        setVaultData((prev) =>
          prev.map((i) => (i.id === id ? { ...i, password: realPassword } : i))
        );
      } else {
  ...
  const handleEdit = async (item: any) => {
    let entryToEdit = item;
    if (item.password === "******") {
      const password = await fetchPassword(item.id);
      if (password) {
        entryToEdit = { ...item, password };
        setVaultData((prev) =>
          prev.map((i) => (i.id === item.id ? { ...i, password } : i))
        );
      }
    }
  ...
  const getCardStyle = (index: number) => {
    ...
    const transform = `translateX(${normalizedOffset * 50}px) translateZ(${-Math.abs(normalizedOffset) * 50
      }px) scale(${scale})`;
                        ...
                        <div className="field-value-box">
                          ...
                          <div className="password-actions">
                            <button
                              className="copy-btn"
                              onClick={() =>
                                handleCopy(item.password, "Password", item.id)
                              }
...
const SecureNotes = () => {
  ...
  return (
    <div className="page-content">
      <div
        className={`secure-notes-container ${selectedNote || isEditing ? "viewing-note" : ""
          }`}
      >
        <ConfirmationModal
          isOpen={isConfirmOpen}
          ...
          <div className="notes-list">
            {isLoading ? (
              ...
              filteredNotes.map((note) => (
                <div
                  key={note.id}
                  className={`note-item ${selectedNote?.id === note.id && !isEditing ? "active" : ""
                    }`}
            ...
            {cards.map((card) => {
              ...
              return (
                <div key={card.id} className="credit-card-wrapper">
                  <div
                    className={`credit-card-item ${isFlipped[card.id] ? "flipped" : ""
                      }`}
...
const Settings = ({ onLogout }) => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.addToast("Failed to fetch audit logs.", "error");
      }
    } catch (error) {
      console.error("Error fetching logs:", error);
      toast.addToast("Error fetching logs.", "error");
    } finally {
  ...
  const checkServerStatus = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setServerId(data.server_id || "Unknown");
      } else {
  ...
  useEffect(() => {
    ...
    if (activeTab === "network") {
      if (!extensionLoaded && loggedInUser) fetchExtensionSettings();
      checkServerStatus();
    }
  }, [activeTab]);
  ...
  const fetchExtensionSettings = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/users/extension/status`, {
        headers: { "X-User-ID": loggedInUser.id },
      });
      if (response.ok) {
        const data = await response.json();
        setExtensionEnabled(data.enabled);
        setExtensionEmail(data.email || "");
  ...
  const handleSaveExtensionSettings = async () => {
    if (extensionEnabled && !extensionEmail) {
      toast.addToast("Email is required when enabled.", "error");
      return;
    }

    setIsSavingExtension(true);
    try {
      const response = await fetch(`${API_BASE_URL}/users/extension/settings`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-User-ID": loggedInUser.id
      ...
      if (response.ok) {
        ...
        toast.addToast(data.error || "Failed to update settings.", "error");
      }
    } catch (error) {
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.addToast(data.error || "Failed to change password.", "error");
      }
    } catch (error) {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        toast.addToast(data.error || "Failed to delete account.", "error");
      }
    } catch (error) {
      toast.addToast("An error occurred. Please try again.", "error");
    } finally {
  ...
  const tabs = [
    { id: "general", label: "General", icon: (props: any) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg> },
    { id: "display", label: "Display", icon: (props: any) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><circle cx="12" cy="12" r="5" /><path d="M12 1v2m0 18v2M4.22 4.22l1.42 1.42m12.72 12.72l1.42 1.42M1 12h2m18 0h2M4.22 19.78l1.42-1.42M18.58 5.42l1.42-1.42" /></svg> },
    { id: "logs", label: "Logs", icon: (props: any) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10 9 9 9 8 9" /></svg> },
    { id: "network", label: "Network", icon: (props: any) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M5 12.55a11 11 0 0 1 14.08 0" /><path d="M1.42 9a16 16 0 0 1 21.16 0" /><path d="M8.53 16.11a6 6 0 0 1 6.95 0" /><line x1="12" y1="20" x2="12.01" y2="20" /></svg> },
    { id: "security", label: "Security", icon: (props: any) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg> },
  ];
  ...
  return (
    <div className="page-content">
        ...
        <div className="settings-content-area">
          {activeTab === "general" && (
            <div className="settings-section">
              ...
              <p className="settings-section-subtitle">Manage your basic account information.</p>

              <div className="settings-row">
                <div className="settings-info">
                  <span className="settings-label">Username</span>
                  <span className="settings-description">Your display name across the application.</span>
                </div>
                <div style={{ fontWeight: "500" }}>{loggedInUser?.username || "N/A"}</div>
              </div>

              <div className="settings-row">
                <div className="settings-info">
                  <span className="settings-label">User ID</span>
                  <span className="settings-description">Unique identifier for your account.</span>
                </div>
                <div style={{ fontFamily: "monospace", fontSize: "0.9rem" }}>{loggedInUser?.id || "N/A"}</div>
              </div>

              <div className="settings-row">
                <div className="settings-info">
                  <span className="settings-label">Language</span>
                  <span className="settings-description">Select your preferred language.</span>
                </div>
          ...
          {activeTab === "display" && (
            <div className="settings-section">
              <h3 className="settings-section-title">Display & Appearance</h3>
              <p className="settings-section-subtitle">Customize how the application looks for you.</p>

              <div className="settings-row">
                <div className="settings-info">
                  <span className="settings-label">Theme Mode</span>
                  <span className="settings-description">Switch between light and dark visual themes.</span>
                </div>
                <button
                  className="btn btn-secondary"
                  onClick={toggleTheme}
              ...
              <div className="settings-row">
                <div className="settings-info">
                  ...
                  <span className="settings-description">Show more items on the screen at once.</span>
                </div>
          ...
          {activeTab === "logs" && (
            <div className="settings-section">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
                <h3 className="settings-section-title" style={{ margin: 0 }}>Audit Logs</h3>
                <div style={{ display: "flex", gap: "0.75rem" }}>
                  <button className="btn btn-secondary" onClick={fetchLogs} disabled={isLoadingLogs} style={{ padding: "0.5rem 1rem", fontSize: "0.85rem" }}>
                    Refresh
                  </button>
                  <button className="btn btn-primary" onClick={downloadLogsCSV} disabled={auditLogs.length === 0} style={{ padding: "0.5rem 1rem", fontSize: "0.85rem" }}>
                    Export CSV
                  </button>
                </div>
              </div>
              <p className="settings-section-subtitle">Security trail of all actions performed on your vault.</p>

              <div className="table-container" style={{ maxHeight: "500px", overflowY: "auto", border: "1px solid var(--border-color)", borderRadius: "12px", background: "var(--background-color)" }}>
                {isLoadingLogs ? (
                  ...
                  <div style={{ padding: "3rem", textAlign: "center", color: "var(--text-secondary-color)" }}>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: "rgba(0,0,0,0.2)", textAlign: "left" }}>
                        <th style={{ padding: "12px 15px", fontSize: "0.85rem", color: "var(--text-secondary-color)" }}>TIME</th>
                        <th style={{ padding: "12px 15px", fontSize: "0.85rem", color: "var(--text-secondary-color)" }}>ACTION</th>
                        <th style={{ padding: "12px 15px", fontSize: "0.85rem", color: "var(--text-secondary-color)" }}>DETAILS</th>
                        <th style={{ padding: "12px 15px", fontSize: "0.85rem", color: "var(--text-secondary-color)" }}>IP ADDRESS</th>
                      </tr>
                    </thead>
                    <tbody>
                      {auditLogs.map((log) => (
                        <tr key={log.id} style={{ borderBottom: "1px solid var(--border-color)" }}>
                          ...
                          <td style={{ padding: "12px 15px", fontSize: "0.85rem", fontWeight: "600" }}>
                            <span style={{
                              padding: "2px 8px",
                              borderRadius: "4px",
                              backgroundColor: log.action.includes('DELETE') ? 'rgba(231, 76, 60, 0.1)' : log.action.includes('CREATE') ? 'rgba(46, 189, 89, 0.1)' : 'rgba(52, 152, 219, 0.1)',
                              color: log.action.includes('DELETE') ? 'var(--accent-color-red)' : log.action.includes('CREATE') ? 'var(--accent-color-green)' : 'var(--accent-color-blue)'
                            }}>
                              {log.action}
                            </span>
                          </td>
                          <td style={{ padding: "12px 15px", fontSize: "0.85rem" }}>{log.details}</td>
                          <td style={{ padding: "12px 15px", fontSize: "0.85rem", fontFamily: "monospace", opacity: 0.8 }}>{log.ip_address}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {activeTab === "network" && (
            <div className="settings-section">
              <h3 className="settings-section-title">Network & Sync</h3>
              <p className="settings-section-subtitle">Configure how the app communicates with the server.</p>

              <div className="settings-row">
                <div className="settings-info">
                  <span className="settings-label">Server Status</span>
                  <span className="settings-description">Current connection status to the backend.</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", color: serverStatus === "connected" ? "var(--accent-color-green)" : "var(--accent-color-red)", fontWeight: "600" }}>
                  <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: serverStatus === "connected" ? "var(--accent-color-green)" : "var(--accent-color-red)" }}></div>
                  {serverStatus === "connected" ? "Connected" : serverStatus}
                </div>
              </div>

              <div className="settings-row">
                <div className="settings-info">
                  <span className="settings-label">System Identity</span>
                  <span className="settings-description">Unique ID for this vault server. Used for extension pairing.</span>
                </div>
                <div style={{
              ...
              <div className="settings-section-divider" style={{ margin: "2rem 0", height: "1px", background: "var(--border-color)" }}></div>

              <h4 style={{ marginBottom: "1rem" }}>Browser Extension Access</h4>
              <p className="settings-section-subtitle">
                Allow the browser extension to access your vault using separate credentials.
                This protects your master password.
              </p>

              <div className="settings-row">
                <div className="settings-info">
                  <span className="settings-label">Enable Extension Access</span>
                  <span className="settings-description">Allow login from the browser extension.</span>
                </div>
                <div style={{ display: "flex", alignItems: "center" }}>
                  ...
                  <style>{`
                    ...
                    input:focus + .slider {
                      box-shadow: 0 0 1px var(--accent-color-blue);
                    }
              ...
              {extensionEnabled && (
                <div style={{
                  ...
                  {/* Outer stretch wrapper — forces the input row to full parent width */}
                  <div style={{
                    position: "relative",
                    width: "100%",
                    display: "flex",
                    ...
                    <input
                      ...
                      onFocus={(e) => {
                        e.currentTarget.style.borderColor = "var(--accent-color-blue)";
              ...
              <div style={{ marginTop: "1.5rem", display: "flex", justifyContent: "flex-end" }}>
                <button
                  className="btn btn-primary"
                  onClick={handleSaveExtensionSettings}
                  disabled={isSavingExtension}
                >
                  {isSavingExtension ? "Saving..." : "Save Extension Settings"}
                </button>
              </div>
            </div>
          )}
          ...
          {activeTab === "security" && (
            <div className="settings-section">
              ...
              <p className="settings-section-subtitle">Manage your vault encryption and account safety.</p>

              <div className="settings-row">
                <div className="settings-info">
                  <span className="settings-label">Master Password</span>
                  <span className="settings-description">Update the password used to encrypt your vault.</span>
                </div>
                <button className="btn btn-secondary" onClick={() => setIsChangePasswordOpen(true)}>Change Password</button>
              </div>

              <div className="settings-row" style={{ marginTop: "2rem", borderTop: "1px solid var(--border-color)", paddingTop: "2rem" }}>
                <div className="settings-info">
                  <span className="settings-label" style={{ color: "var(--accent-color-red)" }}>Danger Zone</span>
                  <span className="settings-description">Permanently delete your account and all vault data.</span>
                </div>
                <button className="btn btn-danger" onClick={() => setIsDeleteAccountOpen(true)}>Delete Account</button>
              </div>
            </div>
          )}
        </div>
      ...
      >
        Are you absolutely sure you want to delete your account? This action is <strong>irreversible</strong> and will permanently delete all your passwords, notes, and credit cards.
      </ConfirmationModal>
    </div>
  );
};
